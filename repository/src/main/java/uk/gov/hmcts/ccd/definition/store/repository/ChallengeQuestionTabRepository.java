...
public interface ChallengeQuestionTabRepository extends JpaRepository<ChallengeQuestionTabEntity, Integer> {
    ...
    @Query("select c from ChallengeQuestionTabEntity c where c.caseType.reference = :caseTypeReference and c.challengeQuestionId = :challengeQuestionId"
        + " and c.caseType.version = (select max(c2.version) from CaseTypeEntity c2 where c2.reference = c.caseType.reference)")
