...
public class CaseFieldEntitySecurityClassificationValidatorImplTest {
    ...
    public void securityClassificationIsNull_invalidValidationResultContainingCaseFieldEntityMissingSecurityClassificationValidationErrorReturned() {
        ...
        assertEquals(
            caseFieldEntity,
            ((CaseFieldEntityMissingSecurityClassificationValidationError) validationResult
    ...
    private void assertCaseFieldWithSecurityClassificationIsValidInContextOfParent(
        ...
        ValidationResult validationResult
            = new CaseFieldEntitySecurityClassificationValidatorImpl().validate(
                caseFieldEntity, caseFieldEntityValidationContext);
        ...
        if (!isValid) {
            ...
            assertEquals(
                caseFieldEntity,
                ((CaseFieldEntityHasLessRestrictiveSecurityClassificationThanParentValidationError) validationResult
                    .getValidationErrors().get(0))
                    .getCaseFieldEntity()
            );
            assertEquals(
