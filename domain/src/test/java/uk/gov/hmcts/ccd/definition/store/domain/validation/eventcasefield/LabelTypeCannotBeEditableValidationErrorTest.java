...
public class LabelTypeCannotBeEditableValidationErrorTest {
    ...
    private LabelTypeCannotBeEditableValidationError classUnderTest;

    @Before
    public void setUp() {
        MockitoAnnotations.initMocks(this);
        when(mockValidationErrorMessageCreator.createErrorMessage(any(LabelTypeCannotBeEditableValidationError.class)))
            .thenReturn(OVERRIDDEN_ERROR_MESSAGE);
        classUnderTest = new LabelTypeCannotBeEditableValidationError(
            eventCaseFieldEntity(caseField("LabelField"),event("EventId"))
        );
    }
    ...
    public void testDefaultMessage() {
        assertEquals(
            "'LabelField' is Label type and cannot be editable for event with reference 'EventId'",
