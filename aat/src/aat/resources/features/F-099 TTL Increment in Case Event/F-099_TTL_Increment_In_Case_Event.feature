@F-099
Feature: F-099: TTL Increment Events column
  ...
  Scenario: Must successfully import a definition file containing a valid value in TTL Increment Events column
    ...
    And a call [fetch a Case Type Schema] will get the expected response as in [S-099.1_Get_CaseType].
  ...
  Scenario: Must successfully import a definition file containing a blank value in TTLIncrement Events column
    ...
    Then a positive response is received
    And the response has all other details as expected
    And a call [fetch a Case Type Schema] will get the expected response as in [S-099.3_Get_CaseType].
  ...
  Scenario: Must successfully import a definition file where TTLIncrement column is missing
    ...
    Then a positive response is received
    And the response has all other details as expected
    And a call [fetch a Case Type Schema] will get the expected response as in [S-099.4_Get_CaseType].
