...
public class ImportController {
    ...
    public static final String IMPORT_JOB_ID_HEADER = "X-Import-Job-Id";

    private final ProcessUploadServiceImpl processUploadServiceImpl;
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(code = 409, message = "An import job with this id already exists"),
    ...
    public ResponseEntity<String> processUpload(
        ...
        if (providedJobIdHeader == null || providedJobIdHeader.isBlank()) {
            ...
            try {
                providedJobId = UUID.fromString(providedJobIdHeader);
            } catch (IllegalArgumentException ex) {
                return ResponseEntity.badRequest()
                    .body("Invalid " + IMPORT_JOB_ID_HEADER + " header: must be a valid UUID");
            }
        }

        ProcessUploadResult result;
        try {
            result = processUploadServiceImpl.processUpload(file, reindex, deleteOldIndex, providedJobId);
        ...
        return ResponseEntity.status(response.getStatusCode())
            ...
            .header(IMPORT_JOB_ID_HEADER, result.jobId().toString())
