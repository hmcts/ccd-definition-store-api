...
public class AccessTypeRolesParser {
    ...
    public List<AccessTypeRolesEntity> parse(final Map<String, DefinitionSheet> definitionSheets,
        ...
        try {
            final List<DefinitionDataItem> accessRoleTypes = definitionSheets
                .get(SheetName.ACCESS_TYPE_ROLES.getName())
                .getDataItems();
            return accessRoleTypes.stream()
                .map(accessRoleType -> createRoleToAccessProfileEntity(parseContext, accessRoleType))
    ...
    private AccessTypeRolesEntity createRoleToAccessProfileEntity(final ParseContext parseContext,
        ...
        accessTypeRolesEntity.setLiveFrom(definitionDataItem.getDate(ColumnName.LIVE_FROM));
        accessTypeRolesEntity.setLiveTo(definitionDataItem.getDate(ColumnName.LIVE_TO));
        accessTypeRolesEntity.setCaseType(caseTypeEntity);
        accessTypeRolesEntity.setAccessTypeID(definitionDataItem.getString(ColumnName.ROLE_NAME));
        ...
        accessTypeRolesEntity.setCaseGroupIdTemplate(definitionDataItem.getString(ColumnName.CASE_GROUP_ID_TEMPLATE));
