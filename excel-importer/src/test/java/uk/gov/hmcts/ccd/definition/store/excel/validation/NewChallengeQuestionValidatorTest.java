...
public class NewChallengeQuestionValidatorTest {
    ...
    public void setup() {
        ...
        buildParseContext();
        newChallengeQuestionValidator = new NewChallengeQuestionValidator(challengeQuestionTabService);
    }
    ...
    public void testAnswerFormatForLongExpression() {
        ...
        buildDefinitionDataItem(CASE_TYPE, FIELD_TYPE, "2", QUESTION_TEXT, DISPLAY_CONTEXT_PARAMETER_1, QUESTION_ID, answer);
        NewChallengeQuestionTabEntity newChallengeQuestionTabEntity = newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
        assertValues(answer, newChallengeQuestionTabEntity, DISPLAY_CONTEXT_PARAMETER_1);
    }

    @Test
    public void testAnswerFormatForSortExpression() {
        String answer = "${OrganisationField.OrganisationName}|${OrganisationField.OrganisationID}:[CLAIMANT]";
        buildDefinitionDataItem(CASE_TYPE, FIELD_TYPE, "2", QUESTION_TEXT, DISPLAY_CONTEXT_PARAMETER_2, QUESTION_ID, answer);
        NewChallengeQuestionTabEntity newChallengeQuestionTabEntity = newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
        assertValues(answer, newChallengeQuestionTabEntity, DISPLAY_CONTEXT_PARAMETER_2);
    }
    ...
    private void assertValues(String answer, NewChallengeQuestionTabEntity newChallengeQuestionTabEntity, String displayContext) {

        assertThat(newChallengeQuestionTabEntity.getAnswerField(), is(answer));
        assertThat(newChallengeQuestionTabEntity.getCaseType().getReference(), is(CASE_TYPE));
        assertThat(newChallengeQuestionTabEntity.getAnswerFieldType().getReference(), is(FIELD_TYPE));
        assertThat(newChallengeQuestionTabEntity.getChallengeQuestionId(), is(QUESTION_ID));
        assertThat(newChallengeQuestionTabEntity.getQuestionText(), is(QUESTION_TEXT));
        assertThat(newChallengeQuestionTabEntity.getDisplayContextParameter(), is(displayContext));
    }

    @Test(expected = InvalidImportException.class)
    public void failForCaseTypeValidation() {
        try {
            buildDefinitionDataItem("incorrectCaseType", FIELD_TYPE, "2", QUESTION_TEXT, DISPLAY_CONTEXT_PARAMETER_1, QUESTION_ID, ANSWERD);
            newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
    ...
    public void failForFieldTypeValidation() {
        try {
            buildDefinitionDataItem(CASE_TYPE, "fiedType-CCC", "2", QUESTION_TEXT, DISPLAY_CONTEXT_PARAMETER_1, QUESTION_ID, ANSWERD);
            newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
        } catch (Exception exception) {
    ...
    public void failForDisplayOrderValidation() {
        try {
            buildDefinitionDataItem(CASE_TYPE, FIELD_TYPE, "TTTT", QUESTION_TEXT, DISPLAY_CONTEXT_PARAMETER_1, QUESTION_ID, ANSWERD);
            newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
        } catch (Exception exception) {
    ...
    public void failForQuestionTextValidation() {
        try {
            buildDefinitionDataItem(CASE_TYPE, FIELD_TYPE, "2", null, DISPLAY_CONTEXT_PARAMETER_1, QUESTION_ID, ANSWERD);
            newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
        } catch (Exception exception) {
    ...
    public void failForDisplayContextParameterValidation() {
        try {
            buildDefinitionDataItem(CASE_TYPE, FIELD_TYPE, "2", QUESTION_TEXT, "HHHHH", QUESTION_ID, ANSWERD);
            newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
        } catch (Exception exception) {
    ...
    public void failForIDValidation() {
        try {
            buildDefinitionDataItem(CASE_TYPE, FIELD_TYPE, "2", QUESTION_TEXT, DISPLAY_CONTEXT_PARAMETER_1, null, ANSWERD);
            newChallengeQuestionValidator.validate(parseContext, definitionDataItem);
        } catch (Exception exception) {
    ...
    private void buildDefinitionDataItem(String caseType, String filedType, String displayOder, String questionText, String displayContextParameter, String id, String answer) {

        definitionDataItem.addAttribute(ColumnName.CASE_TYPE_ID, caseType);
        definitionDataItem.addAttribute(ColumnName.NEW_CHALLENGE_QUESTION_ANSWER_FIELD_TYPE, filedType);
        definitionDataItem.addAttribute(ColumnName.NEW_CHALLENGE_QUESTION_DISPLAY_ORDER, displayOder);
        definitionDataItem.addAttribute(ColumnName.NEW_CHALLENGE_QUESTION_TEXT, questionText);
        ...
        definitionDataItem.addAttribute(ColumnName.NEW_CHALLENGE_QUESTION_ANSWER_FIELD, answer);
