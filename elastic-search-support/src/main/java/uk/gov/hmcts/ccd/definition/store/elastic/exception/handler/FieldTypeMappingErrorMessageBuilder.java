...
public class FieldTypeMappingErrorMessageBuilder extends ElasticsearchErrorMessageBuilder {
    ...
    private static final String REASON_PATTERN = "mapper \\[(.+)\\] of different type, current_type \\[(.+)\\], merged_type \\[(.+)\\]";
    private static final String DATA_PREFIX = "data.";
    private static final String DATA_CLASSIFICATION_PREFIX = "data_classification.";

    private static final String SERVICE_DEFINED_FIELD_TEMPLATE = "Field '%s' in case type '%s' does not match the field type of the field with this id in "
        + "the previous definition version. Please check the field type change is intended and request an Elasticsearch "
        + "reindex. Previous Elasticsearch type was '%s', new type is '%s'.";
    private static final String CCD_METADATA_FIELD_TEMPLATE = "Field '%s' in case type '%s' does not match the expected Elasticsearch type. "
        + "Please request an Elasticsearch reindex for this case type. Previous Elasticsearch type was '%s', new type is '%s'.";
