...
public abstract class ElasticsearchErrorMessageBuilder {
    ...
    public final String buildMessage(ElasticsearchError error) {
        ...
        return buildMessageForError(error);
    }

    protected abstract String buildMessageForError(ElasticsearchError error);

    public final boolean canHandleError(ElasticsearchError error) {
        return error.getReason().matches(reasonPattern);
    }

    protected Matcher getMatcher(String matcherInput) {
        Matcher m = Pattern.compile(reasonPattern).matcher(matcherInput);
        m.matches();
        return m;
    }
}
