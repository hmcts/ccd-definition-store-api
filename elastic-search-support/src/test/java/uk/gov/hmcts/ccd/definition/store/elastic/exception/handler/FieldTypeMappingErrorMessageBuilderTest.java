...
class FieldTypeMappingErrorMessageBuilderTest {
    ...
    void shouldBuildMessageForDataField() {
        ...
        assertAll(
            () -> assertThat(result, is("Field 'data.FieldName' in case type 'CaseTypeId' does not match the field type"
                + " of the field with this id in the previous definition version. Please check the field type change is"
                + " intended and request an Elasticsearch reindex. Previous Elasticsearch type was 'date', new type is"
                + " 'keyword'."))
        );
    }
    ...
    void shouldBuildMessageForDataClassificationField() {
        ...
        assertAll(
            () -> assertThat(result, is("Field 'data_classification.FieldName' in case type 'CaseTypeId' does not match"
                + " the field type of the field with this id in the previous definition version. Please check the field"
                + " type change is intended and request an Elasticsearch reindex. Previous Elasticsearch type was"
                + " 'date', new type is 'keyword'."))
        );
    }
    ...
    void shouldBuildMessageForNonServiceDefinedField() {
        ...
        assertAll(
            () -> assertThat(result, is("Field 'security_classification' in case type 'CaseTypeId' does not match the"
                + " expected Elasticsearch type. Please request an Elasticsearch reindex for this case type. Previous"
                + " Elasticsearch type was 'text', new type is 'keyword'."))
        );
    }
